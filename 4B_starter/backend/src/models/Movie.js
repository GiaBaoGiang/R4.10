...
//Virtual pour récupérer les likes d'un film
...
// 🔥 Middleware pour charger automatiquement les likes
...
// Middleware pour supprimer les likes associés lors de la suppression d'un film
movieSchema.pre("remove", async function (next) {
  try {
    await Like.deleteMany({ movie: this._id });
    next();
  } catch (error) {
    next(error);
  }
